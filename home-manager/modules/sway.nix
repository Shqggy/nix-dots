  ...
  wayland.windowManager.sway = {
    ...
    config = rec {
      ...
      in {
        ...
        # screenshot
        "Print" = ''exec grim -g "$(slurp)" - | wl-copy && notify-send -i camera-photo "snap!" "region copied to clipboard"'';
        "Shift+Print" = ''exec grim - | wl-copy && notify-send -i camera-photo "snap!" "screen copied to clipboard"'';
        "Mod1+Print" = ''exec sh -c 'c=$(hyprpicker -a); notify-send "swish!" "$c ($(pastel format name "$c")) copied to clipboard"' '';
